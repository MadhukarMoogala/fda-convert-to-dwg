    ...
    $('#forgeFiles').jstree({
       ...
       // $("#downloadExport").attr('disabled', 'disabled');
        ...
        if (data.node.type == 'items') {
            ...
            MyVars.fileType = MyVars.filename.slice((MyVars.filename.lastIndexOf(".") - 1 >>> 0) + 2);
            MyVars.projectId = (data.node.original.href).split("/")[6];
            MyVars.itemId = (data.node.original.href).split("/")[8];
...
function statusCallback(completed, message) {
    $(".progressInfo").notify(message, { className: "info", position:"bottom right" });
    $('#exportDWG').prop("disabled", !completed);
  }
...
function addFusionButton(viewer) {
    ...
    button.onClick = function(e){
        if(MyVars.projectId != undefined && MyVars.fileType != undefined){
        ForgeDWG.exportDWG(MyVars.projectId, MyVars.itemId, MyVars.fileName, MyVars.get3LegToken, statusCallback, MyVars.fileType);
        }
    };
    button.addClass('toolbarFusionButton');
    button.setToolTip('Converts CAT file to DWG!');
